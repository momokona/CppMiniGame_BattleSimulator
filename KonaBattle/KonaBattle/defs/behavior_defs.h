...
public:
	...
	{
		ActFunc act_func;
		char input_key{};	// 対応する入力キー
		std::string behavior_message{};
		std::string behavior_name{};
		BehaviorInfo(const ActFunc& ACT_FUNC, char KEY, const std::string& MESSAGE, const std::string& NAME)
			:act_func(ACT_FUNC)
			, input_key(KEY)
	...
	void AddEntry(const BehaviorPattern BEHAVIOR, const ActFunc& ACT_FUNC, char KEY, const std::string& MESSAGE, const std::string& NAME)
	{
	...
	char GetInputKey(const BehaviorPattern BEHAVIOR)
	{
		return behavior_map_.at(BEHAVIOR).input_key;
	}
	const BehaviorPattern GetSelectBehavior(const char INPUT_KEY);
...
{
public:
	RegisterBehaviorInfo(const BehaviorPattern BEHAVIOR, const BehaviorMap::ActFunc& ACT_FUNC, char KEY, const std::string& MESSAGE, const std::string& NAME);
};
